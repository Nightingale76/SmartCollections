...
(function() {
  ...
  async function extractDouyinCollections() {
    ...
    links.forEach((link, index) => {
      ...
      if (
        ...
        const textForTags = (title + ' ' + href).slice(0, 200);
    ...
    if (results.length === 0) {
      ...
      mediaCards.forEach((card, index) => {
        ...
        const textForTags = (title + ' ' + (link?.href || '')).slice(0, 200);
        const tags = window.MEMORA_AI_TAGS && window.MEMORA_AI_TAGS.generateTags
          ? window.MEMORA_AI_TAGS.generateTags(textForTags)
          : ['其他'];
  ...
  async function extractCollectionsFromPage() {
    ...
    if (platform === PLATFORMS.DOUYIN) {
      return await extractDouyinCollections();
    }

    if (platform === PLATFORMS.XIAOHONGSHU) {
      return await extractXhsCollections();
    }
